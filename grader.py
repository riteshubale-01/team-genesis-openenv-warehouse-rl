...
def format_task_score(score: float, task_id: str) -> Dict[str, Any]:
...
def format_tasks_payload(tasks: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for t in tasks:
        task_id = str(t.get("task_id", t.get("difficulty", "unknown")))
        if "score" in t:
            ...
            clean_tasks.append(format_task_score(SCORE_EPSILON, task_id))
